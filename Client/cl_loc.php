...
if(isset($_SESSION['client_id'])){
    ...
    $sql = "SELECT rr.date_debut,rr.date_fin, v.type, v.modele, v.immatriculation, v.prix_jour
            FROM rental_requests rr
            INNER JOIN clients c ON rr.client_id = c.id
            INNER JOIN vehicules v ON rr.vehicule_id = v.id
            WHERE rr.statut = 'en_attente' AND rr.client_id = :c_id";
    $req = $conn->prepare($sql);
    $req->bindParam(':c_id', $client_id);
    $req->execute();
    $demandes = $req->fetchAll(PDO::FETCH_ASSOC);

$date_jour = date('Y-m-d');

    $sql1 = "SELECT l.id, marque, modele, montant_total, date_debut, date_fin, v.type 
    FROM  locations l
    INNER JOIN vehicules v ON l.vehicule_id = v.id
    INNER JOIN clients c ON l.client_id = c.id 
    WHERE l.client_id = :id AND date_fin > '$date_jour' ";  

    $req1 = $conn->prepare($sql1);
    $req1->bindParam(':id', $client_id);
    $req1->execute();
    $locations = $req1->fetchAll(PDO::FETCH_ASSOC);

    $sql2 = "SELECT l.id, marque, modele, montant_total, date_debut, date_fin, v.type 
    FROM  locations l
    INNER JOIN vehicules v ON l.vehicule_id = v.id
    INNER JOIN clients c ON l.client_id = c.id 
    WHERE l.client_id = :id AND date_fin < '$date_jour' ";  
    ...
    $req2->bindParam(':id', $client_id);
    $req2->execute();
    $locations1 = $req2->fetchAll(PDO::FETCH_ASSOC);
}
...
<head>
    ...
    <link rel="stylesheet" href="../CSS/cl_loc.css">
</head>
<body>
    <header class="header">
        <div class="container">
            <div class="header-content">
                    <div class="logo">
                        <a href="../Site/index.php"><img src="../images/footerlogo-removebg-preview.png" alt=""></a>
                    </div>
    
                    <ul class="menu">
                        <li><a href="../Site/index.php">Acceuil</a></li>
                        <li><a href="../Site/vehicule.php">Véhicules</a></li>
                    ...
                    <?php if (isset($_SESSION['client_id'])){ ?>
                        <div class="user-menu-container">
                            ...
                            <ul class="dropdown-menu" id="dropdownMenu">
                                ...
                                <li><a href="cl_loc.php">Mes locations</a></li>
                                <hr>
                                <li>
    ...
    <main class="main" >
        <div class="demande">
            <div class="container">
                <div class="demande-content" style="padding: 50px 0px 30px 0px;">
        ...
        <div class="demande">
            <div class="container">
                <div class="demande-content"style="padding: 0px 0px 50px 0px;">
                            ...
                            <?php } else { ?>
                                <?php foreach ($locations as $location) { ?> 
                                    <tr>
                                        ...
                                        <!-- <td> <a href="Dashboard/rendre.php?id=<?= $location['id'] ?>"> Rendre </a></td> -->
                                    </tr>
                                <?php } ?>
                            <?php } ?> 
                        </tbody>
            ...
            <div class="container">
                <div class="demande-content"style="padding: 0px 0px 50px 0px;">
                    ...
                    <table border="1" cellpadding="15" cellspacing="0">
                        ...
                        <tbody>
                            <?php if (empty($locations1)) { ?>
                                <tr>
                                    <td colspan="5">Aucune location trouvée</td>
                                </tr>
                            <?php } else { ?>
                                <?php foreach ($locations1 as $location) { ?> 
                                    <tr>
                                        <td> <?php echo htmlspecialchars($location['modele']) ?> </td>
                                        <td> <?php echo htmlspecialchars($location['type']) ?> </td>
                                        <td> <?php echo $location['date_debut'] ?> → <?= $location['date_fin'] ?> </td>
        ...
        <div class="container">
            <div class="footer-content">
                ...
                <div class="foot-menu">
                    ...
                    <ul class="lienrap">
                        <li><a href="../Site/index.php">Acceuil</a></li>
                        <li><a href="../Site/vehicule.php">Vehicules</a></li>
